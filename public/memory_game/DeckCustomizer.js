...
function dropHandler(ev) {
  ...
  if (ev.dataTransfer.items) {
    ...
    if (
      candidate.kind === "file" &&
      allowedFileTypes.includes(candidate.type)
    ) {
...
function resetCards() {
  ...
  for (let cardList_idx = 0; cardList_idx < cardsList.length; cardList_idx++) {
    ...
    pair.forEach(card => {
      card.querySelector(".card-front").src =
        imagesArr[customizedCards_idx].src;
      card.querySelector(".card-back").src =
        imagesArr[imagesArr.length - 1].src;
    });
