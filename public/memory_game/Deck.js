class Deck {
  ...
  set setRevealedCard(card) {
    this._revealedCard = card;
  }

  set setDeckName(deckName) {
  ...
  shuffle() {
    ...
    this._cardList.forEach(card => {
      // after we messed out the id's of the card, we match apropirate img to each id
  ...
  unhideAllCards() {
    ...
    setTimeout(
      () => this._cardList.forEach(card => (card.style.visibility = "visible")),
      1050
    );
  }
}
